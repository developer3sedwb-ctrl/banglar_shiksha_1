      ...
      <table id="example" class="table table-striped">
        <thead>
            <tr>
                ...
                <th>Total PRIMARY School's</th>
                <th>Total SECONDARY School's</th>
            </tr>
        </thead>
        <tbody>
            @foreach(($itemList ?? []) as $key=>$item)
              <tr>
                ...
                <td>
                  <a href="{{ in_array($newtype,['district']) ? route('school.contactDetails.district', ['districtid' => $item->id]) : route('school.contactDetails.circle', ['circleid' => $item->id]) }}">
                    {{$item->name??$item->school_name}}
                  </a>
                </td>
                <td>{{$itemWiseCnt[$item->id]->total_primary ?? 0}}</td>
                <td>
                    {{$itemWiseCnt[$item->id]['total_secondary'] ?? 0}}
                    <?php 
                        // if(!empty($itemWiseCnt[$item->id])){
                        //     echo '<pre>';
