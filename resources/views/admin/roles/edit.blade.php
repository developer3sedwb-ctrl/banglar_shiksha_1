                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control" id="description" name="description" placeholder="Enter role description" rows="1">{{ old('description', $role->description) }}</textarea>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        @foreach ($permissions as $group => $groupPermissions)
                                            <div class="col-md-4 mb-4">
                                                <div class="card permission-group-card h-100">
                                                    <div
                                                        class="card-header group-header bg-light d-flex justify-content-between align-items-center">
                                                        <div>
                                                            <h6 class="card-title mb-0 text-capitalize">{{ $group }}
                                                            </h6>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            selectPartialBtn.addEventListener('click', function() {
                permissionCheckboxes.forEach(checkbox => {
                    ...
                    checkbox.checked = commonPermissions.some(perm => label.includes(perm
                        .toLowerCase()));
                });
                updateGroupCheckboxStates();
                updateCardVisualStates();
            ...
            function updateCardVisualState(group) {
                const groupCard = document.querySelector(`.group-checkbox[data-group="${group}"]`).closest(
                    '.permission-group-card');
            ...
            form.addEventListener('submit', function(e) {
                ...
                if (checkedPermissions.length === 0) {
                    if (!confirm(
                            'No permissions selected. Are you sure you want to update this role without any permissions?'
                            )) {
                        e.preventDefault();
                    }
                }
                ...
                if (['Super Admin', 'State Admin'].includes(roleName)) {
                    const superAdminPermissions = document.querySelectorAll('.permission-checkbox:checked')
                        .length;
                    if (superAdminPermissions < permissionCheckboxes.length *
                        0.8) { // At least 80% of permissions
                        if (!confirm(
                                'This is a protected role. Are you sure you want to assign limited permissions?'
                                )) {
                            e.preventDefault();
                        }
                    }
